...
// Endpoint to open project and generate tree
app.get('/api/openProject/:projectId', async (req, res) => {
  ...
  try {
    ...
    const treePath = path.join(projectPath, 'foldertree.json');

    // Check if the manuscript directory exists
    if (!fs.existsSync(manuscriptPath)) {
      return res.status(404).json({ error: 'Manuscript directory not found' });
    }

    // Generate the JSON tree
    const jsonTree = generateFileListWithIds(manuscriptPath);
    saveJsonTree(jsonTree, treePath);

    res.json({ message: 'Project opened successfully', tree: jsonTree });
  } catch (error) {
  ...
  app.post('/api/create_project', async (req, res) => {
    ...
    try {
      ...
      createMetadataFile(projectPath, projectName, authorName, newProject.id, description);
...
// Endpoint to fetch the tree JSON file
app.get('/api/get_tree/:projectId', async (req, res) => {
  const projectId = req.params.projectId;
  // console.log('Fetching tree for project:', projectId);
  try {
    ...
    const treeFilePath = path.join(projectPath, 'foldertree.json');

    // Check if the foldertree.json file exists
    if (!fs.existsSync(treeFilePath)) {
      console.error('Tree file not found:', treeFilePath);
      return res.status(404).json({ error: 'Tree file not found' });
    }

    // Read the foldertree.json file
...
app.post('/api/projects/:projectId/add', async (req, res) => {
  const projectId = req.params.projectId;
  const { type, name, parentPath } = req.body; // Added parentPath

  console.log(`Received request to add item to project: ${projectId}`, req.body);
  ...
  try {
    ...
    const treeFilePath = path.join(projectPath, 'foldertree.json');
    const tree = readJsonFile(treeFilePath) || [];

    const newItemPath = path.join(parentPath || projectPath, name);
    const newItem = {
      id: uuidv4(),
      type,
    ...
    // Create the new item in the directory structure
    if (type === 'folder') {
      fs.mkdirSync(newItemPath);
    } else if (type === 'file') {
      fs.writeFileSync(newItemPath, '');
    }

    // Update the tree structure
    const addItemToTree = (items) => {
    ...
    const updatedTree = parentPath === projectPath ? [...tree, newItem] : addItemToTree(tree);
    writeJsonFile(treeFilePath, updatedTree);
...
app.post('/api/projects/:projectId/delete', async (req, res) => {
  const projectId = req.params.projectId;
  const itemPath = req.body.path; // Corrected to get the itemPath directly

  console.log(`Received request to delete item from project: ${projectId}`, itemPath);
  ...
  try {
    ...
    if (fs.existsSync(itemPath)) {
      ...
      console.log(`Deleted item at path: ${itemPath}`);
    } else {
      console.error(`Item at path ${itemPath} does not exist`);
    ...
    const deleteItemFromTree = (items) => {
      return items.filter((item) => {
        if (item.path === itemPath) return false;
        if (item.children) {
    ...
    const updatedTree = deleteItemFromTree(tree);
    writeJsonFile(treeFilePath, updatedTree);
